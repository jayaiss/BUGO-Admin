...
if (isset($_POST['import_excel'])) {
    ...
    if (!empty($_FILES['excel_file']['tmp_name'])) {
        ...
        try {
            ...
            // First pass: in-file dupes
            foreach ($rows as $i => $row) {
                $email = sanitize_input($row[9] ?? 'N/A');
                if ($email === 'N/A' || $email === '') continue;
                if (isset($processed_emails[$email])) { $duplicate_emails[] = $email; }
            ...
            for ($i = 1; $i < count($rows); $i++) {
                ...
                if ($email === 'N/A' || $email === '' || in_array($email, $duplicate_emails, true)) {
                    continue;
                }

                // Active duplicate (DB) → skip
                ...
                $zone_leader_id = 0;
                $username     = strtolower($email); // username=email in batch
                ...
                if ($first_name && $last_name && $birth_date) {
                    ...
                    // types: ii s s s s s s s s s s s s i s s i i i s s s s s i
                    $stmt->bind_param(
            ...
            if ($duplicates_found) {
                echo "<script>
                    Swal.fire({
                        ...
                        text: 'One or more emails already belong to active residents. Those rows were skipped.',
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['firstName'])) {
    ...
    $usernameField   = sanitize_input($_POST['username'] ?? ''); // <-- NEW: manual username input when no email
    ...
    // Username rule: email preferred, else manual username (required if no email)
    ...
    // Must provide at least email OR username
    if (empty($email) && empty($usernameField)) {
        echo "<script>
            Swal.fire({icon:'error', title:'Missing Login', text:'Please provide either Email or Username for the primary resident.', confirmButtonColor:'#d33'});
        </script>";
        exit();
    }
    ...
    if ($stmt->get_result()->num_rows > 0) {
        echo "<script>
            Swal.fire({icon:'error', title:'Duplicate Entry', text:'Primary resident with the same name already exists (active).', confirmButtonColor:'#d33'});
        </script>";
        exit();
    }
    $stmt->close();
    ...
    if (!empty($email)) {
        ...
        if ($stmt->get_result()->num_rows > 0) {
            echo "<script>
                Swal.fire({icon:'error', title:'Duplicate Email', text:'That email is already used by an active resident.', confirmButtonColor:'#d33'});
            </script>";
            exit();
        }
        $stmt->close();
    ...
    if (!empty($login_username)) {
        $stmt = $mysqli->prepare("SELECT id FROM residents WHERE username = ? AND resident_delete_status = 0 LIMIT 1");
    ...
    try {
        ...
        // types: ii s s s s s s s s s s s s i s s i i i s s s s s
        $stmt->bind_param(
        ...
        if (isset($_POST['family_firstName']) && is_array($_POST['family_firstName'])) {
            ...
            $family_usernames       = $_POST['family_username'] ?? []; // <-- NEW: manual username for child
            ...
            for ($i = 0; $i < count($family_first_names); $i++) {
                ...
                // Username rule for child
                $fam_login_username = !empty($fam_email) ? strtolower($fam_email) : strtolower($fam_username_f);
                if (empty($fam_email) && empty($fam_username_f)) {
                ...
                // active-only dupes for username
                if (!empty($fam_login_username)) {
                    ...
                    $chk->execute();
                    if ($chk->get_result()->num_rows > 0) {
                        throw new Exception("Child #".($i+1).": Username already exists for an active resident.");
                    }
                    $chk->close();
                }
...
<script>
    ...
    document.getElementById('searchResidentInput').addEventListener('input', function () {
...
document.addEventListener("DOMContentLoaded", function () {
  const form = document.getElementById("addResidentForm"); // replace with your form ID
  if (form) {
...
// Also re-run when modal is opened (in case fields are refilled)
