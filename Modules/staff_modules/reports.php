...
function printReport() {
  ...
  if (type === "cases") {
    ...
    win.onload = () => setTimeout(() => win.print(), 500);
  } else if (type === "residents") { // NEW: clean print for Residents
    const age    = document.getElementById("residentAge")?.value || "";
    const gender = document.getElementById("residentGender")?.value || "";
    const zone   = document.getElementById("residentZone")?.value || "";
    const win = window.open(
      `load_report/load_residents_report.php?print=1&age=${encodeURIComponent(age)}&gender=${encodeURIComponent(gender)}&res_zone=${encodeURIComponent(zone)}`,
      '_blank'
    );
    win.onload = () => setTimeout(() => win.print(), 500);
  } else {
    // fallback: default window print
    window.print();
  }
}
</script>
