...
/* ---------------------------------------------
   ...
   Force redirect to BESO list after import (success or error)
---------------------------------------------- */
if (
    ...
    try {
        ...
        if ($ext === 'xlsx') {
            ...
            if (class_exists(\PhpOffice\PhpSpreadsheet\IOFactory::class)) {
                ...
                foreach ($sheet->toArray(null, true, true, false) as $idx => $r) {
                    ...
                    // [0]=firstName, [1]=middleName, [2]=lastName, [3]=suffixName, [4]=education_attainment, [5]=course
                    $rows[] = [ s50($r[0] ?? ''), s50($r[1] ?? ''), s50($r[2] ?? ''), s50($r[3] ?? ''), s50($r[4] ?? ''), s50($r[5] ?? '') ];
                }
                $loaded = true;
            }
            if (!$loaded) {
                @unlink($dest);
                throw new RuntimeException('PhpSpreadsheet not installed. Please upload CSV or install via Composer.');
            }
        } else { // CSV
            $fh = fopen($dest, 'r');
            ...
            if ($first !== false) {
                ...
                if (!$isHeader) $rows[] = [ s50($cols[0] ?? ''), s50($cols[1] ?? ''), s50($cols[2] ?? ''), s50($cols[3] ?? ''), s50($cols[4] ?? ''), s50($cols[5] ?? '') ];
            }
            while (($data = fgetcsv($fh)) !== false) {
                if (count(array_filter($data, fn($x) => $x !== null && $x !== '')) === 0) continue;
                $rows[] = [ s50($data[0] ?? ''), s50($data[1] ?? ''), s50($data[2] ?? ''), s50($data[3] ?? ''), s50($data[4] ?? ''), s50($data[5] ?? '') ];
            }
            fclose($fh);
        }
        @unlink($dest);
        ...
        $stmt = $mysqli->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 0, NOW())
        ");
        if (!$stmt) throw new RuntimeException('DB error (prepare): ' . $mysqli->error);

        $employeeId = (int)($_SESSION['employee_id'] ?? 0);
        $inserted = 0; $skipped = 0;
        foreach ($rows as $r) {
            [$f, $m, $l, $suf, $edu, $course] = $r;
            if ($f === '' || $l === '') { $skipped++; continue; } // require first+last
            $stmt->bind_param('ssssssi', $f, $m, $l, $suf, $edu, $course, $employeeId);
            if (!$stmt->execute()) { error_log('[BESO Import][Row Skip] ' . $stmt->error); $skipped++; continue; }
            $inserted++;
        }
        $mysqli->commit();
        $stmt->close();

        // Force redirect back to BESO list (ignore $redirects['beso'] if it points elsewhere)
        $redirectURL = (function_exists('enc_beso') ? enc_beso('beso') : 'index_beso_staff.php?page=beso');

        // NOTE: Do NOT set $_SESSION['swal'] here to avoid duplicate popup after redirect.

        // Inline SweetAlert + redirect
        ...
        // On error, still send user back to BESO list
        $redirectURL = (function_exists('enc_beso') ? enc_beso('beso') : 'index_beso_staff.php?page=beso');
...
<?php
// Page-load SweetAlert flash reader (kept for other flows; this import no longer sets it)
  ...
  <div class="card shadow-sm mb-4">
    ...
    <div class="card-body p-0">
      <div class="table-responsive" style="overflow-y: auto; max-height: 600px; overflow-x: hidden;">
        <form method="GET" action="index_beso_staff.php" class="mb-3">
           <input type="hidden" name="page" value="<?= $_GET['page'] ?? 'beso' ?>">
            ...
            <div class="row g-2 align-items-end">
                ...
                <a href="<?= function_exists('enc_beso') ? enc_beso('beso') : 'index_beso_staff.php?page=beso' ?>" class="btn btn-secondary w-100">Clear</a>
                </div>
            </div>
        </form>
          ...
          <tbody>
            ...
            if (!empty($result) && $result instanceof mysqli_result && $result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                // Support snake_case or camelCase field names from the SELECT
  ...
  <div class="modal-dialog">
    <form method="POST" enctype="multipart/form-data" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <!-- IMPORTANT: send redirect back to BESO list (not dashboard) -->
        <input type="hidden" name="return_to" value="<?= htmlspecialchars(function_exists('enc_beso') ? enc_beso('beso') : 'index_beso_staff.php?page=beso') ?>">
        <div class="mb-3">
